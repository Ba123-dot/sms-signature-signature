...
int main()
{
    ...
    uint64_t start_gen_sign_cpu = __rdtsc();
    clock_t start_gen_sign = clock();
    for (int t = 0; t < times; t++)
    {
        MatrixXd Y(m, 1);
        for (int k = 0; k < m; k++)
    ...
    uint64_t end_gen_sign_cpu = __rdtsc();
    clock_t end_gen_sign = clock();
    ...
    // 获取开始的 CPU cycles
    uint64_t start_verify_cpu = __rdtsc();
    ...
    // 获取结束的 CPU cycles
    uint64_t end_verify_cpu = __rdtsc();
    clock_t end_verify = clock();

    // 输出gen_key、gen_sign、verify-cpucycles
    uint64_t gen_key_time_cpu =
        (end_gen_key_cpu - start_gen_key_cpu) / double(times); // CLOCKS_PER_cpucycles;
    cout << "gen_key_time_cpu:" << endl;
    cout << gen_key_time_cpu << "cpucycles" << endl;

    uint64_t gen_sign_cpu_cycles = (end_gen_sign_cpu - start_gen_sign_cpu) /
                                   double(times); // / CLOCKS_PER_cpucycles;
    cout << "gen_signature_time_cpu:" << endl;
    cout << gen_sign_cpu_cycles << "cpucycles" << endl;

    uint64_t verify_cpu_cycles =
        (end_verify_cpu - start_verify_cpu) / double(times); // CLOCKS_PER_cpucycles;
    cout << "verify_time_cpu:" << endl;
    cout << verify_cpu_cycles << "cpucycles" << endl;

    // 输出gen_key、gen_sign、verify-times(um)
    double gen_key_time =
        (end_gen_key - start_gen_key) / double(times); // CLOCKS_PER_SEC;
    cout << "gen_key_time:" << endl;
    cout << gen_key_time << "um" << endl;

    double gen_signature_time = (end_gen_sign - start_gen_sign) /
                                double(times); // / CLOCKS_PER_SEC;
    cout << "gen_signature_time:" << endl;
    cout << gen_signature_time << "um" << endl;

    double verify_time =
        (end_verify - start_verify) / double(times); // / CLOCKS_PER_SEC;
    cout << "verify_time:" << endl;
    cout << verify_time << "um" << endl;
    return 0;
}